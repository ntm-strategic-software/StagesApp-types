...
/**
 * NewTranscription is Transcription with _id, createdAt, and updatedAt optional.
 * The intention is, If not provided, they will be set by the database when the row is created/updated in the database.
 */
export interface NewTranscription extends Omit<Transcription, '_id' | 'createdAt' | 'updatedAt'> {
  /** Unique ID for the Transcription */
  _id?: string
  /**
   * ISO Date (e.g., '2022-06-20T15:50:40.055Z'), when the row was initially saved to the database.
